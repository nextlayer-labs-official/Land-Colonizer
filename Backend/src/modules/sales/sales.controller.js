...
function sanitize(body) {
  ...
  const d = {
    ...
    attorney_completed:        body.attorney_completed     === true || body.attorney_completed     === 'true' || false,
    full_final_completed:      body.full_final_completed   === true || body.full_final_completed   === 'true' || false,
...
const INCLUDE = {
  inventory: { select: { id: true, inventory_code: true, type: true, plot_no: true, sl_no: true, location: true,
    ...
    area: true, area_unit: true, facing: true, rate: true, status: true,
...
async function createSale(req, res) {
  ...
  if (!data.sold_by_id) {
    data.sold_by_id   = req.user?.id   ?? null;
    data.sold_by_name = req.user?.name ?? null;
  }
